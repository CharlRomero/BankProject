...
string InputData<string>::readString(int typ) {
	cout << "\nIngrese su numero de cuenta: ";
	cin >> value;
	while (check.checkAccount(value, typ)) {
		cout << "\nNumero de cuenta incorrecto, vuleva a ingresar: ";
		cin >> value;
	}
	return value;
}
string InputData<int>::readString(string message) {
	cout << message;
	cin >> value;
	while (!check.floatDouble(value)) {
		cout << "\nNumero de cuenta incorrecto, vuleva a ingresar: ";
